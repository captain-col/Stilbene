...
int analyzeTimes(int filenumberstart, int filenumberend, int runT, bool bl);
...
int analyzeTimes(int filenumberstart, int filenumberend, int runT, bool bl = false)
{	
	b = 1;
	evCounter = 0; 
	char filename[90];
	...
	h24 = new TH1D("flux_over_time", "Flux over (Computer) Run Time", ceil(runT/10), 0, runT);
	...
	h25 = new TH1D("flux_pb","Flux Measured per Bunch", 100, 0, h24->GetMaximum());
	l->Add(h25);
	for(int i = 0; i< ceil(runT/10); i++){
	  h25->Fill(h24->GetBinContent(i));
	}
	...
	TFile* f = new TFile(Form("runtime_histos_%d_%d.root",filenumberstart, filenumberend),"recreate");
	TIter iter(l);
	TH1D* h;
...
void FillHistos(event_t &ev){
  ...
  //  printf("Start time tenths ms %ld\n", (long)ev.clockTimeTenthsMs);
